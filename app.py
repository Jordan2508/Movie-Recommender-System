...
def fetch_poster(title):
    url = f"https://api.themoviedb.org/3/search/movie?api_key=f43230da1c21f2ef6f6f45c55f5fc51a&language=en-US&query={title}"
    ...
    try:
        ...
        if data.get("results"):
            poster_path = data["results"][0].get("poster_path")  # ✅ inside results[0]
            if poster_path:
                print(poster_path)
...
def recommend(movie):
    ...
    for i in distances[1:6]:
        title = movies.iloc[i[0]].get('title')
        poster_url = fetch_poster(title)

        recommended_movie_names.append(title)   # ✅ now appending names
